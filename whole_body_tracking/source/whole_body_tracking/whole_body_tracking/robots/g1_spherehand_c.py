...
G1_29DOF_TORSOBASE_CFG.spawn = sim_utils.UrdfFileCfg(
    ...
    #asset_path=os.path.join(__file_dir__, "resources/unitree_g1/urdf/g1_29dof_torsobase_equiv.urdf"), 
...
# -----------------------------------------------------------------------
# URDF joint limit reading utilities (same pattern as g1_spherehand.py)
# -----------------------------------------------------------------------
G1_SPHEREHAND_C_URDF_PATH = f"{ASSET_DIR}/g1_spherehand_c/g1_29dof_spherehand_c.urdf"


def _read_urdf_joint_limits(asset_path: str) -> dict[str, dict[str, float]]:
    ...
    for joint in root.findall("joint"):
        ...
        if joint_name is None or effort is None or velocity is None:
            raise RuntimeError(f"Joint {joint_name!r} in {asset_path} has an incomplete <limit> tag")
...
G1_JOINT_LIMITS = _read_urdf_joint_limits(G1_SPHEREHAND_C_URDF_PATH)


def _joint_limits_for(patterns: list[str], field: str) -> dict[str, float]:
    limits = {}
    for pattern in patterns:
        regex = re.compile(pattern)
        values = {
            joint_limits[field]
            for joint_name, joint_limits in G1_JOINT_LIMITS.items()
            if regex.fullmatch(joint_name)
        }
        if not values:
            raise RuntimeError(f"No joints in URDF match {pattern!r}")
        if len(values) != 1:
            raise RuntimeError(f"Joints matching {pattern!r} have different {field} limits: {sorted(values)}")
        limits[pattern] = values.pop()
    return limits


# Joint grouping (matching g1_spherehand.py pattern)
LEG_JOINTS = [
    ".*_hip_yaw_joint",
    ".*_hip_roll_joint",
    ".*_hip_pitch_joint",
...
FOOT_JOINTS = [".*_ankle_pitch_joint", ".*_ankle_roll_joint"]
WAIST_JOINTS = ["waist_roll_joint", "waist_pitch_joint"]
WAIST_YAW_JOINTS = ["waist_yaw_joint"]
ARM_JOINTS = [
    ".*_shoulder_pitch_joint",
    ".*_shoulder_roll_joint",
    ".*_shoulder_yaw_joint",
...
G1_29DOF_PELVISBASE_POPSICLE_CFG = ArticulationCfg(
    spawn=sim_utils.UrdfFileCfg(
        fix_base=False,
        replace_cylinders_with_capsules=True,
        asset_path=f"{ASSET_DIR}/g1_spherehand_c/g1_29dof_spherehand_c.urdf",
        activate_contact_sensors=True,
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            disable_gravity=False,
            retain_accelerations=False,
...
# -----------------------------------------------------------------------
# Action scale computation (same pattern as g1_spherehand.py)
# -----------------------------------------------------------------------
G1_ACTION_SCALE = {}
for a in beyondmimic_g1_29dof_actuators.values():
    e = a.effort_limit_sim
